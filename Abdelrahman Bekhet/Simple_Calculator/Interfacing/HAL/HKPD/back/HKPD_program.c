...
void hkpd_getPressedButton(u8_t* pu8_button)
{
    ...
    *pu8_button = KPD_DEFAULT_VALUE;
    ...
    {
         /*Setting the rows as input pullup*/
    mdio_setPinStatus(KPD_PORT,(KPD_COL0 | KPD_COL1 | KPD_COL2 | KPD_COL3) , INPUT_PULLUP);

    /*Setting the columns as output*/
    mdio_setPinStatus(KPD_PORT, (KPD_ROW0 | KPD_ROW1 | KPD_ROW2| KPD_ROW3), OUTPUT);

    /*Setting the columns as output high*/
    mdio_setPinValue(KPD_PORT,(KPD_ROW0 | KPD_ROW1 | KPD_ROW2) , HIGH);

    mdio_setPinValue(KPD_PORT,( KPD_ROW3) , LOW);

    //Looping to see which col is pressed
        for(au8_column = 0; au8_column < KPD_NUM_COLS; au8_column++)
        {   
            
            /*Getting the row pin value*/
           
            mdio_getPinValue(KPD_PORT, (KPD_COL0 << au8_column), &au8_returnValue);
            ...
            if(au8_returnValue == LOW)
            {
                ...
                /*Getting the pressed key value*/
                *pu8_button = garr_keypadMapping[3][au8_column];
